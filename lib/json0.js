  ...
  arrayDiff(before, after, deepEqual).forEach(function(diff) {
    ...
    if (diff.type === 'remove') {
      ...
      if (diff.howMany <= diff.from - diff.to) {
        for (i = 0; i < diff.howMany; i++) op.push({p: path.concat(diff.from + i), lm: diff.to + i});
      } else {
        for (i = 0; i < diff.from - diff.to; i++) op.push({p: path.concat(diff.to), lm: diff.from + diff.howMany - 1});
      }
      var block = work.splice(diff.from, diff.howMany);
      work.splice.apply(work, [diff.to, 0].concat(block));
    }
  });
...
function diffObject(path, before, after) {
  ...
  Object.keys(after).forEach(function(key) {
    if (!hasOwn(before, key)) {
      ...
      op.push.apply(op, diffValue(path.concat(key), before[key], after[key]));
    }
  });
  return op;
}
