...
function diffArray(path, before, after) {
  ...
  arrayDiff(before, after, deepEqual).forEach(function(diff) {
    if (diff.type === 'remove') {
      ...
      // A move relocates a run of `howMany` elements from base index `from` to `to`. json0 has no
      // block move, so we emit one `lm` per element - but the same reshape can be expressed two ways:
      // slide the run to its destination (`howMany` lms), or slide the `from - to` displaced elements
      // the other way past it (that many lms). Pick whichever needs fewer.
