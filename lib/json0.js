...
// Produce an op that transforms `before` into `after`. Strings are diffed
// down to a single contiguous change (sd/si) so the op composes and transforms
// cleanly past concurrent edits, rather than replacing the whole value.
...
function diffValue(path, before, after) {
  ...
  if (typeof before === 'string' && typeof after === 'string') {
    return diffString(path, before, after);
  }
...
function diffString(path, before, after) {
  var start = 0;
  ...
  var op = [];
  var deleted = before.slice(start, endBefore);
  var inserted = after.slice(start, endAfter);
  if (deleted) op.push({p: path.concat(start), sd: deleted});
  if (inserted) op.push({p: path.concat(start), si: inserted});
  return op;
}
